...
router.post("/contact.html",function(req,resp){
	...
	if(verifNameOk && verifEmailOk && verifsujetOk){
			...
			// Insertion en DB
			mysqlhelper.pool.query(sql,[name,email,subject, 1],function(err,rows,fields){
				
		// Si il y a une erreur, message erreur
		if(err != null)
		{			
			message = "Il y a eu une erreur, le message n'a pas été envoyé";
			console.log('ECHEC ENVOI FORM ---> '+message + ' ' + err);
		}
		// Sinon, message réussite et envoi du mail de bonne réception du formulaire à l'utilisateur (Simulation avec mon mail)
			else
			{
	             ...
	             var mailoptions ={                    
	                     ...
	                     //format html
	                     html :"<p>Bonjour, Nous vous remercions pour  votre confiance, votre demande vient d’être traitée par notre équipe et aurez de nos nouvelles dans les plus brefs délais.L’équipe Coffee-cup</p>"
	                     //option obligatoire sinon il ne marche pas
	             }
	             transporter.sendMail(mailoptions,function(err,info){
	                 //si j'ai une erreur je l'écris dans la console le return permet de sortir de la fonction et de ne pas afficher le suivant
	                 if(err!= null) return console.log('ECHEC ENVOI MAIL DE CONFIRMATION DE BONNE RECEPTION DU MESSAGE VIA LE FORMULAIRE ----->'+err);
	                 
	                 //test echec envoi: mettre mauvais mot de passe dans le config.js
		                 else{
		                	 // Envoi mail OK insérer statut envoyé
